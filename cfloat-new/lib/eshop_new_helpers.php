...
/**
 * Dohledá sklad + EAN + ID varianty pro položku objednávky. Pokud položka
 * nese idProductItem (přímé ID konkrétní varianty - nejspolehlivější cesta),
 * dotáže se na ni přímo. Jinak spadne na starší postup: kód produktu (number)
 * + porovnání velikosti (z pole "dial", případně oldVariantValue1) proti
 * variantám podle kódu.
 * Vrací ['stock' => int|null, 'exact' => bool, 'ean' => string|null, 'product_item_id' => string|null].
 */
...
{
    if ($number === '') return ['stock' => null, 'exact' => false, 'ean' => null, 'product_item_id' => null];
    ...
    foreach ($members as $pi) {
        $s = (int)($pi['stock'] ?? 0);
        if ($maxStock === null || $s > $maxStock) { $maxStock = $s; $maxStockId = basename((string)($pi['@id'] ?? '')); }
        ...
        foreach (($pi['productVariantDefinitionList'] ?? []) as $def) {
            ...
            if (eshop_new_norm_size($dialVal) === $wanted) {
                ...
                $exactId = basename((string)($pi['@id'] ?? ''));
                break 2;
            }
        }
    }

    $result = $exactStock !== null ? ['stock' => $exactStock, 'exact' => true, 'ean' => $exactEan, 'product_item_id' => $exactId]
        : ($wanted === '' ? ['stock' => $maxStock, 'exact' => true, 'ean' => null, 'product_item_id' => $maxStockId] : ['stock' => $maxStock, 'exact' => false, 'ean' => null, 'product_item_id' => null]);
    $stockCache[$cacheKey] = $result;
    return $result;
}
...
/**
 * Jen ZJISTÍ (nic nezapisuje), jestli existuje přesně jedna jiná varianta se
 * stejným EAN, a jaký by byl nový sklad. Používá se pro náhled před potvrzením.
 */
function eshop_new_find_duplicate_candidate(string $baseUrl, string $token, string $ean, string $ownProductItemId, int $pieces): array
{
    if ($ean === '') return ['count' => 0, 'candidate' => null];
    $res = eshop_new_api_call($baseUrl, $token, '/api-engine/product-items?ean=' . urlencode($ean));
    $members = $res['body']['hydra:member'] ?? [];
    $others = [];
    foreach ($members as $pi) {
        $piId = basename((string)($pi['@id'] ?? ''));
        if ($piId !== '' && $piId !== $ownProductItemId) $others[] = $pi;
    }

    // Mezi "jinými" ignorujeme další VÝPRODEJ duplikáty (typicky mají stejné
    // jméno začínající "VÝPRODEJ" - to jsou další kopie stejné výprodejové
    // položky, ne to, co nás zajímá). Zajímá nás vždy ten NEVÝPRODEJOVÝ
    // (normální) protějšek.
    $nonSale = array_values(array_filter($others, function ($pi) {
        return mb_stripos((string)($pi['name'] ?? ''), 'výprodej', 0, 'UTF-8') === false && stripos((string)($pi['name'] ?? ''), 'vyprodej') === false;
    }));

    if (count($nonSale) !== 1) return ['count' => count($nonSale), 'candidate' => null];
    ...
    return [
        'count' => 1,
        'candidate' => [
            'id' => basename((string)($dup['@id'] ?? '')),
            'name' => (string)($dup['name'] ?? ''),
            'old_stock' => $oldStock,
            'new_stock' => max(0, $oldStock - $pieces),
        ],
    ];
}

/**
 * Skutečně zapíše nový sklad do jedné konkrétní varianty (PATCH). Volá se až
 * po potvrzení náhledu.
 */
function eshop_new_write_stock(string $baseUrl, string $token, string $productItemId, int $newStock): bool
{
    $ch = curl_init($baseUrl . '/api-engine/product-items/' . urlencode($productItemId));
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
...
function eshop_new_sync_duplicate_stock(PDO $pdo, string $baseUrl, string $token, string $orderIri, string $itemIri, ?string $ean, ?string $ownProductItemId, int $pieces): void
{
    ...
    $candidates = array_values(array_filter($candidates, function ($pi) {
        return mb_stripos((string)($pi['name'] ?? ''), 'výprodej', 0, 'UTF-8') === false && stripos((string)($pi['name'] ?? ''), 'vyprodej') === false;
    }));

    // Bezpečnostní brzda: pokud se nenajde přesně jedna jiná (nevýprodejová) varianta, radši nic neděláme.
...
/**
 * Uloží kompletní data objednávky (základ + položky se skladem) do DB.
...
function eshop_new_persist_order(PDO $pdo, string $baseUrl, string $token, array $order): void
{
    ...
    $itemUpsert = $pdo->prepare("
        INSERT INTO eshop_new_order_items (order_iri, item_iri, name, number, size, pieces, price_with_vat, stock, stock_exact, ean, id_product_item, updated_at)
        VALUES (:o, :i, :name, :number, :size, :pieces, :price, :stock, :exact, :ean, :idpi, NOW())
        ON DUPLICATE KEY UPDATE
            ...
            ean = VALUES(ean), id_product_item = VALUES(id_product_item), updated_at = NOW()
    ");
    foreach ($items as $it) {
        ...
        $itemUpsert->execute([
            ...
            ':idpi' => $stockInfo['product_item_id'] ?? (!empty($it['idProductItem']) ? (string)$it['idProductItem'] : null),
        ]);
        // POZOR: oprava duplicitního skladu se už NESPOUŠTÍ automaticky zde -
        // dělá se ručně přes zaškrtnutí + náhled + potvrzení, viz nove-objednavky.php.
    }
}
