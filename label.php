<?php
// Přístup pro přihlášené UŽIVATELE (prohlížeč) NEBO pro QZ Tray (tiskárna na
// počítači obsluhy), který tuhle URL volá přímo přes internet bez session/cookie
// – proto se navíc povoluje i sdílený token (stejný mechanismus jako u cronu),
// který cfloat-new/tisk-stitku.php připojuje k labelUrl/labelZplUrl.
...
function gls_extract_parcel_number_from_response($data): string {
    // "ParcelNumber" je SKUTEČNÉ číslo balíku (to, co je na štítku a v portálu
    // GLS). "ParcelId" je jen GLS interní ID záznamu a NENÍ to tracking –
    // proto se hledá přednostně a výhradně "parcelnumber", ne obě dohromady.
    $exactCandidates = [];
    $fallbackCandidates = [];
        ...
        if (is_array($cur)) {
            foreach ($cur as $k => $v) {
                if (is_string($k)) {
                    $kl = strtolower($k);
                    if ($kl === 'parcelnumber') {
                        if (is_string($v) || is_numeric($v)) $exactCandidates[] = (string)$v;
                    } elseif (in_array($kl, ['parcelno', 'trackingnumber', 'tracking', 'parcelcode'], true)) {
                        if (is_string($v) || is_numeric($v)) $fallbackCandidates[] = (string)$v;
                    }
                    // "parcelid" schválně VYNECHÁNO – je to jen interní GLS ID, ne tracking.
                }
                if (is_array($v)) {
                    $stack[] = $v;
                }
    ...
    $candidates = !empty($exactCandidates) ? $exactCandidates : $fallbackCandidates;
...
function gls_create_label_pdf($orderData, $glsCountry, $forceNew = false, $typeOfPrinter = null) {
    ...
    // DOČASNÉ LADĚNÍ – uložíme syrovou odpověď GLS přímo do stejného cache
    // souboru (gls_cache.json), o kterém jistě víme, že se do něj zapisovat
    // dá. Bezpečné smazat, jakmile problém vyřešíme.
    // Pole "Labels" obsahuje binární PDF (obrovské pole bajtů) – to z ladicího
    // výpisu vynecháme, jinak je nečitelný.
    if (function_exists('gls_cache_load') && function_exists('gls_cache_save')) {
        $glsDebugCache = gls_cache_load();
        $glsDebugOrderNo = (string)($orderData['order_number'] ?? '');
        if ($glsDebugOrderNo !== '') {
            if (!isset($glsDebugCache[$glsDebugOrderNo]) || !is_array($glsDebugCache[$glsDebugOrderNo])) {
                $glsDebugCache[$glsDebugOrderNo] = [];
            }
            $glsDebugDataForLog = $data;
            if (isset($glsDebugDataForLog['Labels'])) {
                $glsDebugDataForLog['Labels'] = '(vynecháno – binární PDF, ' . count((array)$glsDebugDataForLog['Labels']) . ' položek)';
            }
            $glsDebugCache[$glsDebugOrderNo]['debug_raw_response'] = $glsDebugDataForLog;
            $glsDebugCache[$glsDebugOrderNo]['debug_extracted_pn'] = $pn;
            gls_cache_save($glsDebugCache);
        }
    }
