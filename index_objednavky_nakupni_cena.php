...
if ($loggedIn && $view === 'returns') {
    ...
    $rows = loadCsvRows(RETURNS_SHEET_CSV_URL);

    if ($rows === false) {
        $returnsError = 'Nepodařilo se načíst data pro vrácené zboží.';
    } else {
        $returnsByKey = [];
        ...
        foreach ($rows as $i => $cols) {
            ...
            $rowDateRawFull = isset($cols[RETURNS_DATE_COL_INDEX]) ? (string)$cols[RETURNS_DATE_COL_INDEX] : '';
            $rowDateObj     = $rowDateRawFull !== '' ? parseSheetDate($rowDateRawFull) : null;
            ...
            $colJ = isset($cols[RETURNS_J_COL_INDEX]) ? trim((string)$cols[RETURNS_J_COL_INDEX]) : '';
            if ($colJ === '') continue;

            $orderVal = isset($cols[2]) ? trim((string)$cols[2]) : '';
            $kVal     = isset($cols[RETURNS_K_COL_INDEX]) ? trim((string)$cols[RETURNS_K_COL_INDEX]) : '';
            ...
            $colD = isset($cols[RETURNS_D_COL_INDEX]) ? trim((string)$cols[RETURNS_D_COL_INDEX]) : '';
            $colI = isset($cols[RETURNS_I_COL_INDEX]) ? trim((string)$cols[RETURNS_I_COL_INDEX]) : '';
            $colE = isset($cols[RETURNS_E_COL_INDEX]) ? trim((string)$cols[RETURNS_E_COL_INDEX]) : '';
            $colL = isset($cols[RETURNS_L_COL_INDEX]) ? trim((string)$cols[RETURNS_L_COL_INDEX]) : '';
            $colM = isset($cols[RETURNS_M_COL_INDEX]) ? trim((string)$cols[RETURNS_M_COL_INDEX]) : '';

            $qty = 0.0;
            if ($colL !== '') {
                $numL = str_replace(' ', '', $colL);
                $numL = str_replace(',', '.', $numL);
                if (is_numeric($numL)) {
                    $qty = (float)$numL;
                }
            }
            ...
            if (!isset($returnsByKey[$key])) {
                $returnsByKey[$key] = [
                    'date'    => $rowDateYmd ?? $rowDateRawFull,
                    'd'       => $colD,
                    'j'       => $colJ,
                    'i'       => $colI,
                    'e'       => $colE,
                ...
                if ($rowDateObj && $rowDateObj->getTimestamp() > $ref['date_ts']) {
                    ...
                    $ref['date']    = $rowDateYmd ?? $rowDateRawFull;
                }
                unset($ref);
            }
        }
                            ...
                            <table class="stats-table orders-table">
                                ...
                                <?php foreach ($ordersRows as $row): ?>
                                    ...
                                    <tr class="order-main-row" data-order-id="<?php echo $idOrder; ?>" onclick="toggleOrderDetail(<?php echo $idOrder; ?>);">
                                        ...
                                        <td class="orders-col-order"><?php echo $orderNumber !== '' ? h($orderNumber) : '—'; ?></td>
                    ...
                    <div class="table-wrap">
                        <table class="stats-table">
                            ...
                            foreach ($returnsRows as $row):
                                ...
                                <tr id="<?php echo h($detailId); ?>" class="stats-detail-row">
                                    <td colspan="6" class="stats-detail-cell">
