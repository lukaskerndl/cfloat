...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => 'vavrys', 'msg' => 'Soubor nebyl odeslán.']);
}

$supplierKey = uvav_safe_supplier_key((string)($_POST['supplier'] ?? 'vavrys'));
if (!isset($supplierConfigs[$supplierKey])) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => 'vavrys', 'msg' => 'Neznámý dodavatel.']);
}
$cfg = $supplierConfigs[$supplierKey];
$anchor = $cfg['anchor'] ?? 'vavrys-aktualizace';
...
if (!uvav_ensure_dir($vavrysAbsDir)) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Nelze vytvořit nebo otevřít složku /' . UVAV_FOLDER . '/.'], $anchor);
}

$postedName = uvav_safe_filename((string)($_POST['vavrys_var_file'] ?? $cfg['var_file']));
if (strcasecmp($postedName, $cfg['var_file']) !== 0) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Tento modul zpracovává jen soubor ' . $cfg['var_file'] . '.'], $anchor);
}

$uploadedVarAbs = uvav_find_fixed_file($cfg['var_file'], $supplierKey, $scriptRoot, $docRoot, $vavrysAbsDir);
if (!$uploadedVarAbs || !is_file($uploadedVarAbs)) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Chybí soubor ' . $cfg['var_file'] . ' ve složce /' . UVAV_FOLDER . '/.'], $anchor);
}

$mainAbs = uvav_find_fixed_file($cfg['main_file'], $supplierKey, $scriptRoot, $docRoot, $vavrysAbsDir);
if (!$mainAbs || !is_file($mainAbs)) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Chybí hlavní soubor ' . $cfg['main_file'] . ' ve složce /' . UVAV_FOLDER . '/.'], $anchor);
}

$lookupFile = uvav_find_merged_sklad_file($scriptRoot, $docRoot);
if (!$lookupFile || !is_file($lookupFile)) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Nejdřív vytvoř SpojeneFeedy_CSTORE_SKLAD.csv přes nahrání skladu C-Store.'], $anchor);
}

$stamp = date('Ymd_His');
$outName = uvav_output_name($cfg['var_file'], $stamp);
$outAbs = $vavrysAbsDir . '/' . $outName;
$mainOutName = uvav_output_name($cfg['main_file'], $stamp);
...
uvav_snapshot_previous_output_for_changes($vavrysAbsDir, $cfg['var_file'], $supplierKey, 'var');
uvav_snapshot_previous_output_for_changes($vavrysAbsDir, $cfg['main_file'], $supplierKey, 'main');

// Každé zpracování vytvoří nový název souboru a starší výstupy stejného dodavatele smaže.
uvav_delete_old_outputs($vavrysAbsDir, $cfg['var_file'], $outName);
uvav_delete_old_outputs($vavrysAbsDir, $cfg['main_file'], $mainOutName);

$lookup = [];
$lookupDelim = uvav_detect_delim_from_file($lookupFile);
$fhLookup = @fopen($lookupFile, 'rb');
if (!$fhLookup) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Spojený soubor se skladem nejde otevřít.'], $anchor);
}
$lookupRows = 0;
...
if (!$lookup) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Ve SpojeneFeedy_CSTORE_SKLAD.csv nejsou žádné použitelné EAN hodnoty.'], $anchor);
}

$varDelim = uvav_detect_delim_from_file($uploadedVarAbs);
$fhVar = @fopen($uploadedVarAbs, 'rb');
if (!$fhVar) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => '_var soubor nejde otevřít.'], $anchor);
}
...
if (!uvav_ensure_dir($tmpDir)) {
    fclose($fhVar);
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Nelze vytvořit dočasnou složku pro řazení.'], $anchor);
}
...
if ($header === null) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => '_var soubor neobsahuje hlavičku.'], $anchor);
}
...
if (!$fhMain) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Hlavní soubor nejde otevřít.'], $anchor);
}
...
$mainZeroRows = 0;
$mainOneRows = 0;
$mainHiddenProducts = 0; // změna v hlavním souboru: 0 -> 1
$mainOpenedProducts = 0; // změna v hlavním souboru: 1 -> 0
$mainLineNo = 0;
$mainHeader = null;
$mainEanColumns = [];
...
if ($mainHeader === null) {
    uvav_redirect(['vavrys_update' => 'err', 'supplier' => $supplierKey, 'msg' => 'Hlavní soubor neobsahuje hlavičku.'], $anchor);
}
...
uvav_redirect([
    ...
    'found' => (string)$foundRows,
    'missing' => (string)$missingRows,
    'main_rows' => (string)$mainRows,
    'main_zero' => (string)$mainZeroRows,
    'main_one' => (string)$mainOneRows,
    'hidden_products' => (string)$mainHiddenProducts,
    'opened_products' => (string)$mainOpenedProducts,
], $anchor);
